...
# 6. Generate a style-consistent image set directly from one semantic prompt
ID=$(chatgpt-agent new --initial "生成一套 4 张方形咖啡品牌社媒图: 门店吧台、拿铁特写、咖啡豆包装、早晨外带杯。保持同一视觉系统。" | head -1)
chatgpt-agent send "$ID" "沿用同一视觉系统,再生成 2 张: 甜点搭配和冰咖啡。"
```
...
For image sets, do not look for a special batch command. Ask for the full set
in natural language with `new`, then use the returned conversation id with
`send` for follow-up images. The CLI downloads every image ref returned by
ChatGPT and prints one `[image] <path>` line per generated image.
